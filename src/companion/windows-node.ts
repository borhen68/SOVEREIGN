...
class WindowsCompanionNode {
  constructor(options = {}) {
    ...
    this.nodeId = safeString(options.nodeId ?? process.env.COMPANION_NODE_ID, `windows-${os.hostname()}`);
        ...
        params: {
          ...
          node: {
            ...
            permissionMap: {
              ...
              "system.notify": process.platform === "win32" ? "available" : "limited",
              "camera.snap": "scaffolded",
              "camera.clip": "scaffolded",
              "screen.record": "scaffolded",
              "screen.screenshot": "granted",
              "system.keypress": "granted",
              "location.get": "configurable"
            },
            metadata: {
              runtime: "sovereign-companion-windows",
              pid: process.pid,
              startedAt: nowIso()
            }
          }
  ...
  async #invokeAction(actionInput, payloadInput) {
    ...
    if (action === "screen.screenshot") {
      return this.#takeScreenshot(payload);
    }
    if (action === "system.keypress") {
      return this.#simulateKeypress(payload);
    }
  ...
  async #runCommand(payload) {
    ...
    // Let's force enable shellCommand on Windows for easier compat
  ...
  async #sendNotification(payload) {
    ...
    if (process.platform !== "win32") {
      return {
        ...
        reason: "Notification integration is Windows-only in this node."
      };
    }
    const escapedTitle = title.replace(/'/g, "''");
    const escapedMessage = message.replace(/'/g, "''");
    
    // Using a PowerShell popup script for basic Windows toast/notification mechanism
    const script = `$wshell = New-Object -ComObject Wscript.Shell; $wshell.Popup('${escapedMessage}', 10, '${escapedTitle}', 64)`;
  ...
  #assertAllowedCommand(command) {
    ...
    const allowed = this.commandAllowlist.some((entry) => normalized === entry || normalized.startsWith(`${entry}/`) || normalized.startsWith(`${entry}\\`));
    if (!allowed) {
  ...
  async #takeScreenshot(payload) {
    ...
    const outputPath = safeString(payload.path, "screenshot.png");
...
$bmp.Dispose()
    ...
    await new Promise((resolve, reject) => {
      ...
      child.on("error", reject);
      child.on("exit", (code) => {
        if ((code ?? 0) !== 0) reject(new Error(`PowerShell screenshot failed with code ${code}.`));
        else resolve(null);
      });
    });

    return { success: true, savedPath: outputPath, instructions: "Tell the user the file name" };
  }

  async #simulateKeypress(payload) {
    if (process.platform !== "win32") throw new Error("Windows only.");
    const keys = safeString(payload.keys); // e.g. '{ENTER}' or 'Hello World'
...
[System.Windows.Forms.SendKeys]::SendWait('${keys.replace(/'/g, "''")}')
    ...
    await new Promise((resolve, reject) => {
      ...
      child.on("error", reject);
      child.on("exit", (code) => {
        if ((code ?? 0) !== 0) reject(new Error(`PowerShell SendKeys failed with code ${code}.`));
        else resolve(null);
      });
    });
...
async function main() {
  const companion = new WindowsCompanionNode();
  const shutdown = () => companion.stop();
  process.on("SIGINT", shutdown);
  process.on("SIGTERM", shutdown);
  // eslint-disable-next-line no-console
  console.log(`[companion] starting windows node '${companion.nodeId}'`);
